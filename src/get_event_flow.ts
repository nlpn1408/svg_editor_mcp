...
export const getEventFlowTool = {
  ...
  handler: async (args: { eventName: string }) => {
    ...
    try {
      ...
      if (emitterLines.length > 0) {
        ...
        if (emitterLines.length > 15) lines.push(`   ... and ${emitterLines.length - 15} more`);
      } else {
      ...
      if (subscriberLines.length > 0) {
        ...
        if (subscriberLines.length > 15) lines.push(`   ... and ${subscriberLines.length - 15} more`);
      } else {
