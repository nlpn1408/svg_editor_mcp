...
export const searchCodeTool = {
  ...
  handler: async (args: { query: string, layer?: string, fileType?: string }) => {
    const { query, layer = 'all', fileType = 'all' } = args;
    
    // Determine search path based on layer
    let searchPath: string;
    if (layer && layer !== 'all' && ARCHITECTURE_LAYERS[layer as keyof typeof ARCHITECTURE_LAYERS]) {
      searchPath = ARCHITECTURE_LAYERS[layer as keyof typeof ARCHITECTURE_LAYERS];
    } else {
