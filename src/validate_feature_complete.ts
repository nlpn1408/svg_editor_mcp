    ...
    try {
      const checks = await Promise.all([
        ...
        checkByContent(
          ...
          [`class ${pascal}Entity`, `class ${pascal}\\b`],
          ['src/domain/entities'],
          ['*.ts']
        ),
        // 2. IRepository interface
        checkByContent(
          `IRepository (I${pascal}Repository)`,
        ...
        // 4. Mapper
        checkByContent(
          `Mapper (*${lower}*-mappers.ts)`,
          [`fromDomainToExternal|fromExternalToDomain`],
          ['src/infrastructure'],
          [`*${lower}*mappers*`, `*${lower}*mapper*`]
        ),
        // 5. API Service
        checkByContent(
          `API Service (API${pascal}Service)`,
          [`class API${pascal}Service`, `class.*${pascal}.*Service extends APIBaseService`],
          ['src/editor/services'],
          ['*.ts']
        ),
        // 6. Zustand Store
        checkByContent(
          `Zustand Store (use${pascal}Store)`,
          [`use${pascal}Store`, `${pascal}Store`],
          ['src/editor/stores', 'src/editor/components'],
          ['*.ts', '*.tsx']
        ),
        // 7. Container component
        checkByContent(
          `Container (${pascal}Container)`,
          [`${pascal}Container`],
          ['src/editor/components', 'src/features'],
          ['*.tsx', '*.ts']
        ),
        // 8. Presenter component
        checkByContent(
          `Presenter (${pascal}Presenter)`,
          [`${pascal}Presenter`],
          ['src/editor/components', 'src/features'],
          ['*.tsx', '*.ts']
        ),
