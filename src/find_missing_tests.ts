...
async function hasTestFile(baseName: string): Promise<boolean> {
    ...
    try {
      if (scope === 'domain' || scope === 'all') {
        ...
        for (const file of entityFiles) {
          ...
          if (!baseName || baseName === 'BaseEntity') continue;
          const hasTest = await hasTestFile(baseName);
          if (!hasTest) missing.push({ type: 'Domain Entity', file, baseName });
        }
        ...
        for (const file of serviceFiles) {
          ...
          const hasTest = await hasTestFile(baseName);
          if (!hasTest) missing.push({ type: 'Domain Service', file, baseName });
        }
      }
      ...
      if (scope === 'commands' || scope === 'all') {
        ...
        for (const file of commandFiles) {
          ...
          if (!baseName || baseName === 'History' || baseName === 'Command' || !baseName.includes('Command')) continue;
          const hasTest = await hasTestFile(baseName);
          if (!hasTest) missing.push({ type: 'Command', file, baseName });
        }
      }
